...
public class Test2
{
	...
	public static void main(String[] args)
	{
	    BlockingQueue<String> queue = new ArrayBlockingQueue<String>(16,true);
		for (int i = 0; i < 4; i++)
		{
		 ...
		 * 为了解决这个问题先将日志放入一个阻塞队列，然后4个线程从阻塞队列中取
		 */
		for (int i = 0; i < 16; i++)
		{ // 这行代码不能改动
			final String log = "" + (i + 1);// 这行代码不能改动
...
public class Test1
{
	...
	public static void main(String[] args)
	{
		final Semaphore semaphore = new Semaphore(1);//同步信号
		final SynchronousQueue<String> queue = new SynchronousQueue<String>(true);//同步阻塞队列
		/*
		 ...
		 * 
		 * 为了保证上一个消费者消费完后，下一个消费者才能消费数据了，必须同步
		 */
		for (int i = 0; i < 10; i++)
		{
			new Thread(new Runnable()
...
public class Test3 extends Thread
{
	...
	static class TestDo
	{
		...
		{
			Object o = key;
			if (!keys.contains(o))//对象的Equals方法
			{
				keys.add(o);
			}
			else
			{
				o=keys.get(keys.indexOf(o));
